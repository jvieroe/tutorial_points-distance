...
# ---------------------------------------------------------
chargers_pts <- chargers %>% 
  dplyr::select(c(charger_id, geometry)) # nrow = N
...
# ggplot() +
#   geom_sf(data = zones) +
#   geom_sf(data = centroids, size = 0.0001, color = "red") # worked out fine

# ----- Create distance matrix of N x K dimensions
dist_unclass <- st_distance(chargers_pts, centroids,
                            by_element = FALSE) %>% 
  unclass() %>% 
  as.data.frame()

# check
(nrow(chargers_pts)*nrow(centroids)) - (nrow(dist_unclass)*ncol(dist_unclass))
...
# check
(nrow(chargers_pts)*nrow(centroids)) - (nrow(dist_unclass))
